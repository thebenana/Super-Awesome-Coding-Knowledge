...
void parseArgs(char *buffer, char** args, int argsSize, int *nargs) {
  ...
  int j = 0; //only use j with while loop
  int i = 0;
  
  while (((bufArgs[j] = strsep(&buffer, " \n\t")) != NULL) && ((bufArgs[j] == '\0') || ((j + 1) < argsSize))) { //you want to assign the values to buffer, incrementing j
    j++;
  }

  *nargs = 0; //point to 0 by default
 
  while (bufArgs[i] != NULL) {
    if (strlen(bufArgs[i]) > 0) {
      args[*nargs] = bufArgs[i]; //set buffer to variables taken by parameter
      (*nargs) = (*nargs) + 1; //increment nargs
    }
    i++; //use i as a stopper
  }
